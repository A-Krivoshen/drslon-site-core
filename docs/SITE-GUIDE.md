...
### Тема vs плагин — кто за что отвечает

| Что | Тема `drslon-blog-theme` | Плагин `drslon-site-core` |
|-----|--------------------------|---------------------------|
| Шапка, подвал, меню | ✅ | — |
| Блог, архивы, single пост (шаблоны) | ✅ | — |
| Шорткоды блога (слайдер, секции, related) | — | ✅ |
...
## 2. Тема WordPress `drslon-blog-theme`
...
| Лайтбокс картинок в постах | `assets/js/content-lightbox.js` |

### Деплой темы

```bash
cd /var/www/krivoshein.site/htdocs/wp-content/themes/drslon-blog-theme
git add -A && git commit -m "описание" && git push origin main
```

После смены CSS/JS темы — сброс кеша (раздел 7), т.к. стили в каждой странице.
...
## 3. Карта страниц → шорткод → где править
...
## 4. ACF-экраны в админке WordPress
...
## 5. Прайс-лист `/prays-list/` — как устроен
...
### 5.2 Структура страницы (сверху вниз)
...
### 5.4 Воронка конверсии (куда ведут кнопки)
...
## 9. Чего НЕ делать (частые ловушки)
...
## 12. Структура плагина (дерево файлов)
...
## 13. Быстрые ответы «что делать если…»
...
## 15. Что можно сделать потом (не срочно)
